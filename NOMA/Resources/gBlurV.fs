...
void main(void)
{
   ...
   sum += texture2D(RTBlurH, vec2(vTexCoord.x, vTexCoord.y - 7.0*blurSize)) * 0.005;
   sum += texture2D(RTBlurH, vec2(vTexCoord.x, vTexCoord.y - 6.0*blurSize)) * 0.01;
   sum += texture2D(RTBlurH, vec2(vTexCoord.x, vTexCoord.y - 5.0*blurSize)) * 0.02;
